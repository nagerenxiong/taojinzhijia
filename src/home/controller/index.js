...
export default class extends Base {
  ...
  async indexAction() {
    //获取置顶文章
    let topList = await this.model("home").getArticleList({
      totop: 1,
      ispublished: 1
    });
    ...
    //获取近期活动
    let actList = await this.model("home").getArticleList({
      item: 6,
      ispublished: 1
    });
  ...
  async pageAction() {
      ...
      if (blogInfo.ispublished === 1) {
        // 设置浏览量加1
        let viewcount = await this.model("home").addViewCount({
          id: aid
        });

        //获取标签名
        let tagItem = await this.model("home").findOne("tags", {
          id: blogInfo.tag
        });
        ...
        //您可能感兴趣文章
        let relatearticle = await this.model("home").getArticleList({
          tag: blogInfo.tag,
          ispublished: 1
        })
  ...
  async postdigAction() {
      ...
      let rs = await this.model("home").updateRecord("comment", {
        id: id
      }, newData);
      if (rs) {
        return this.success();
  ...
  async getList(itemId, menu) {
    ...
    //分页
    let itemList = await this.model("home").getPageSelect({
      item: itemId,
      ispublished: 1
    }, pagenumber, pagesize);
    ...
    let item = await this.model("home").findOne("item", {
      id: itemId
    });
  ...
  async categoryAction() {
    ...
    this.assign('pageData', pageData);
    //分页
    let category = await this.model("home").findOne("tags", {
      id: itemId
    });
